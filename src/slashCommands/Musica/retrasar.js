...
module.exports = {
    ...
    .setDescription('🎧 Retrasa segundos en la cancion')
    .addIntegerOption(option => option
        .setName("seconds")
        .setDescription("Cantidad de segundos que quieres retrasar")
        .setMinValue(0)
        .setRequired(true)   
    ),
    async execute(client, interaction) {
        ...
        try {
            ...
            embed.setColor(process.env.COLOR).setDescription(`⏪ La canción se retrasó \`${seconds}s\`.`);
            return interaction.reply({ embeds: [embed], ephemeral: true});
